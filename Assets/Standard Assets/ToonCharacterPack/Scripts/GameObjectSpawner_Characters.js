...
function OnGUI () {
	...
	if(GUI.Button(Rect(170,(maxButtons+1)*18,100,18),"Clear Bodies")){
	...
	if(GUI.Button(Rect(170,(maxButtons+2)*18,100,18),"Decapitate")){
	
		this.currentGO.GetComponent(ToonCharacterController).Decapitate(false, 1, Vector3.zero);
	
	
	}
	if(GUI.Button(Rect(170,(maxButtons+3)*18,100,18),"Explode Head")){
	
		this.currentGO.GetComponent(ToonCharacterController).Decapitate(true, 0, Vector3(3,10,3));
	
	
	}
	if(GUI.Button(Rect(170,(maxButtons+4)*18,100,18),"Blink Random")){
	
		this.currentGO.GetComponent(ToonCharacterController).Blink(10, .05);
	
	
	}
	if(GUI.Button(Rect(170,(maxButtons+5)*18,100,18),"Blink White")){
	
		this.currentGO.GetComponent(ToonCharacterController).Blink(10, .05, 20, 20, 20);
	...
	for(var i:int=0;i < pageButtonCount;i++){
		...
		if(GUI.Button(Rect(20,i*18+18,250,18),buttonText)){
			if(currentGO){
			this.currentGO.GetComponent(ToonCharacterController).enableRagdoll(0,Vector3(3,10,3));
			currentGO = null;
			}
